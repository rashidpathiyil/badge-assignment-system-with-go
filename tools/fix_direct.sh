#!/bin/bash
# Script to directly fix the remaining test criteria issues

echo "Fixing remaining test criteria issues with direct approach..."

# Fix 1: Fix the criteria nesting in rule_engine_test.go
# This is an acceptable pattern since the event-based criteria documented format:
# {
...
#   }
# }
# So we're not changing it, but documenting it as intentional
echo "Creating a note about criteria nesting in rule_engine_test.go"
cat > /tmp/rule_engine_note.txt << 'EOL'
// Note: The criteria format used here follows the standard event-based criteria format:
// {
//   "event": "event_type_name",
//   "criteria": {
//     "field_name": {
...
//   }
// }
// This format is documented in docs/TEST_CRITERIA_EXAMPLES.md
EOL

sed -i.bak "/testCriteria :=/i\\$(cat /tmp/rule_engine_note.txt)" internal/engine/rule_engine_test.go

# Fix 2: Directly fix the pattern_test.go by creating a new version
echo "Directly fixing the pattern_test.go file"
cat > /tmp/fix_pattern_test.go << 'EOL'
...
cp /tmp/fix_pattern_test.go tests/legacy/engine/pattern_criteria/pattern_test.go

# Fix 3: Fix the time_evaluators_test.go with a direct edit
echo "Creating a fixed version of time_evaluators_test.go using direct edits"
# Directly fixing the TestPatternCriteria function, which seems to be the issue
sed -i.bak '/TestPatternCriteria/,/^}/ s/criteria := map\[string\]interface{}{\s*\n.*"pattern":/criteria := map[string]interface{}{\n\t\t"$pattern": map[string]interface{}{\n\t\t\t"pattern":/' internal/engine/time_evaluators_test.go
sed -i.bak '/TestPatternCriteria/,/^}/ s/"maxDeviation": float64(0.1),/"maxDeviation": float64(0.1),\n\t\t},/' internal/engine/time_evaluators_test.go

echo "Direct fixes completed. The following files have been modified:"
echo "1. internal/engine/rule_engine_test.go - Added documentation note about criteria nesting"
echo "2. tests/legacy/engine/pattern_criteria/pattern_test.go - Completely replaced with fixed version"
echo "3. internal/engine/time_evaluators_test.go - Directly fixed pattern criteria format"
