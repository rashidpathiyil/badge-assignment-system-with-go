...
// TestLogicalOperatorFormat tests badge assignment using the logical operator format
func TestLogicalOperatorFormat(t *testing.T) {
	// Create log file
	logFile, err := os.Create("logical_operator_test.log")
	if err != nil {
	...
	log("Starting logical operator format test")
	...
	eventTypeReq := utils.EventTypeRequest{
		...
		Description: "Event with score for testing logical operators",
	...
	// Step 3: Create a Badge with logical operator format criteria
	log("Step 3: Creating badge with logical operator format criteria")

	// Instead of "steps", use a logical operator format
	logicalFlowDefinition := map[string]interface{}{
		// Use the standard MongoDB-like query format with operators
		"$and": []map[string]interface{}{
			{
				...
				"condition": map[string]interface{}{
					...
					"params": map[string]interface{}{
						"threshold": 10, // Very low threshold for testing
					},
				},
			},
		},
	...
	userID := fmt.Sprintf("logical-user-%d", timestamp)
