...
// TestTimestampFilterBadge tests a badge that uses timestamp filtering as an alternative to $timeWindow
// This badge rewards users who have been active in the last 30 days with at least 5 activities
func TestTimestampFilterBadge(t *testing.T) {
	SetupTest()
	...
	testUserID := "test_user_timestamp" // Consistent user ID for this test
	...
	// STEP 2: Create a badge with timestamp filtering
	// Calculate 30 days ago for the timestamp filter
	now := time.Now()
	...
	flowDefinition := map[string]interface{}{
		...
		"criteria": map[string]interface{}{
			...
			"timestamp": map[string]interface{}{
				"$gte": thirtyDaysAgo.Format(time.RFC3339), // Only count events from the last 30 days
			},
		},
	}
	...
	// Optionally, decode the metadata to verify the event_count
	if badgeMetadata != "" {
		var decodedMetadata map[string]interface{}
		err := json.Unmarshal([]byte(badgeMetadata), &decodedMetadata)
		if err == nil {
			if eventCount, ok := decodedMetadata["event_count"].(float64); ok {
				t.Logf("Event count from metadata: %.0f", eventCount)
				// We expect only the recent events to be counted due to the timestamp filter
				assert.Equal(t, float64(len(recentActivities)), eventCount,
					fmt.Sprintf("Expected event_count to be %d in metadata", len(recentActivities)))
			}
		}
	}
}
